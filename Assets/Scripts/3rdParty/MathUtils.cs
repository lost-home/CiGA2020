...
public static class MathUtils {
    ...
    public static T RandomEnum < T > () where T : System.Enum {
        return RandomArrayElement ( ( T[] )System.Enum.GetValues ( typeof ( T ) ) );
    }
    ...
    public static float InertialDamp ( float previousValue, float targetValue, float smoothTime, float deltaTime ) {
        float x        = previousValue - targetValue;
        float newValue = x + deltaTime * ( -1f / smoothTime * x );
