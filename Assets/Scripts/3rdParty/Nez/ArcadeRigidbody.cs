...
namespace Nez
{
	...
	public class ArcadeRigidbody : MonoBehaviour
	{
		/// <summary>
		/// delta time layer
		/// </summary>
		...
		/// </summary>
		/// <param name="value">Value.</param>
		public ArcadeRigidbody setGravityScale ( float value ) {
			_gravityScale = value;
			return this;
		}
		...
		{
			if( !isImmovable )
				velocity += force * 100000 * ( _inverseMass * CupheadTime.Delta[timeLayer] * CupheadTime.Delta[timeLayer] );
		}
		...
		{
			if( !isImmovable )
				velocity += force * ( _inverseMass * CupheadTime.Delta[timeLayer] * CupheadTime.Delta[timeLayer] );
		}
		...
		void processOverlap( ArcadeRigidbody other, ref Vector2 minimumTranslationVector )
		{
			if( isImmovable )
			{
				other.transform.position += (Vector3)minimumTranslationVector;
			}
			else if( other.isImmovable )
			{
				transform.position -= (Vector3)minimumTranslationVector;
			}
			else
			{
				transform.position -= (Vector3)minimumTranslationVector * 0.5f;
				other.transform.position += (Vector3)minimumTranslationVector * 0.5f;
			}
		}
