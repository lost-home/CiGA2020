...
public class AudioPlayManager : MonoBehaviour
{
    ...
    {
        a2DPlayer.SetMusicVolume(masterVolume * musicVolume);
        a3DPlayer.SetMusicVolume(masterVolume * musicVolume);
    }

    private static void SetSFXVolume()
    {
        a2DPlayer.SetSFXVolume(masterVolume * sfxVolume);
        a3DPlayer.SetSFXVolume(masterVolume * sfxVolume);
    }

    public static void SaveVolume()
    {
        //RecordManager.SaveRecord("GameSettingData", "TotleVolume", MasterVolume);
	...
	public static void PlaySFX2D ( string[] names, float volumeScale = 1f, float delay = 0f, float pitch = 1, string flag = "" ) {
		a2DPlayer.PlaySFX ( MathUtils.RandomArrayElement ( names ), volumeScale, delay, pitch, flag );
	}
