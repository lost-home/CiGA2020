...
namespace Characters.Movements {
	...
	public class PushInfo {
		...
		[SerializeField] internal bool ignoreOtherForce;
		[SerializeField] internal bool expireOnGround;

		[SerializeField] internal PushForce force1;
		[SerializeField] internal Curve curve1;
		[SerializeField] internal PushForce force2;
		[SerializeField] internal Curve curve2;

		internal (Vector2, Vector2) Evaluate ( Transform from, ITarget to ) {
			return ( force1.Evaluate ( from, to ), force2.Evaluate ( from, to ) );
		}

		// internal (Vector2, Vector2) Evaluate ( Projectile from, ITarget to ) {
		// 	return ( force1.Evaluate ( from, to ), force2.Evaluate ( from, to ) );
		// }
